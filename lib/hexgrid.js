...
function Point(x, y) {
  ...
  function hexArea(hex, radius, boardRadius) {
    ...
    let ctr = Hex(0, 0, 0);
    let result = [];
    let offset, offsetq, offsets, offsetr, rStart, rEnd, h;
    offset = hexGetCoord(hex);
  
    offsetq = offset[0]; //0
    offsetr = offset[1]; //-1
    offsets = offset[2]; //1, radius 2
  ...
  function hexArea2(hex, radius, boardRadius) {
    if (!isHex(hex)) throw `Input ${hex} is not a valid hex`;
    //hex is center
    for (let q = -radius + hex.q; q <= radius + hex.q; q++) {
      //rStart =
    }
  }
  ...
  //becomes hexDraw
  function hexDraw(layout, hex, color) {
    //layout, hex
  ...
  function hexDrawArray(layout, hexes, hex_color) {
    for (var i = 0; i < hexes.length; i++) {
      hexDraw(layout, hexes[i], hex_color[i]);
    }
  }
  ...
  function hexGenerateBoard(radius, hexes, offset = Hex(0, 0, 0), hex_color) {
    ...
    for (var i = 0; i < hexes.length; i++) {
      hex_color.push(color(random(255), random(255), random(255)));
    }
  }
